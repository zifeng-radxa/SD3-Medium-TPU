...
model_transform.py --model_name mmdit_$name --input_shape $shape --model_def $name"_0".onnx --mlir $name.mlir
...
#     model_deploy.py --mlir $name.mlir --quantize W4BF16 --chip bm1684x $quant --model $name.bmodel --quantize_table quant.mlir
#     rm -rf `ls | grep -v '.bmodel$' | grep -v '.mlir$' | grep -v '.onnx$' | grep -v '.pt$'  | grep -v 'profile_0.txt$' `
# done
# name=tail
# shape=[[1,77,4096]]
